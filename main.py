...
def create_tk_images(root, pil_assets):
    ...
    if missing_faces: print(f"Warning: Missing Tk face images for: {', '.join(missing_faces)}")
...
def main():
    ...
    # 4. Combine Assets
    # 'assets' now contains pil_assets (including dimensions) and tk_assets (cards + dice)
    ...
    # 9. Prepare Deck (logic remains the same as previous step)
    ...
    for card in full_deck:
        ...
        if is_jack: jacks_removed_count += 1
        elif is_two_of_clubs: removed_count += 1
        else: deck_for_grid.append(card)
    ...
    expected_grid_deck_size = (config.ROWS * config.COLUMNS) - 1
    if len(deck_for_grid) != expected_grid_deck_size: exit(f"FATAL ERROR: Deck size for grid ({len(deck_for_grid)}) doesn't match expected ({expected_grid_deck_size}). Check logic.")

    # 10. Deal Cards (logic remains the same as previous step)
        ...
        for c in range(config.COLUMNS):
            if r == center_r and c == center_c:
                ...
                if card_index < len(deck_for_grid):
                    ...
                    card_state_grid[r][c] = config.STATE_FACE_DOWN
                    card_index += 1
                else: card_state_grid[r][c] = config.STATE_ACTION_TAKEN
    total_dealt = card_index + 1
    expected_total = config.ROWS * config.COLUMNS
    print(f"- Successfully placed all {total_dealt} cards.") if total_dealt == expected_total else print(f"- Warning: Placed {total_dealt} cards, but expected {expected_total}.")

    # 11. Create Grid Buttons (pass the full 'assets' dict)
        ...
        for c in range(config.COLUMNS):
            ...
            if card is not None and current_state == config.STATE_FACE_DOWN:
                click_command = lambda row=r, col=c: game_logic.handle_card_click(
                    ...
                    assets, # Pass the full assets dict including dice Tk images
                    info_frame_bg
                )
                button = tk.Button(grid_frame, image=tk_photo_back, command=click_command, borderwidth=0, highlightthickness=0, relief=tk.FLAT, bg=button_bg, activebackground=button_bg, state=tk.NORMAL)
